...
function Card({ id, isCompleted }) {
  ...
  return (
    <Link className='relative transition-all duration-300 border rounded-lg border-slate-300 dark:border-slate-600 pointer hover:border-blue-600 ease' to={`/detail/${id}`}>
      ...
      <div className='p-4 pt-2'>
        ...
        <h4 className='text-base font-semibold text-slate-600 dark:text-slate-400'>Rp{formatNumber(target)}</h4>
        {!isCompleted && (
